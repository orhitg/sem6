...
/************************** Error Handler ***************************/
void CheckError(const int return_value, const char *function, bool exit_process = true)
{
	if(return_value < 0)
	{
		cout<<"\n ERROR!!!";
...
int TCP_ChatServer(int client_desc)
{
	...
	while((msg_len = read(client_desc, msg, BUFFER_SIZE)) != 0) 
	{
		cout<<"[User "<<client_desc<<"] Msg Recieved :"; fflush(stdout);
		write(fileno(stdout), msg, msg_len);

		if(msg[0] == 'b' && msg[1] == 'y' && msg[2] == 'e')
			return 0;
	
		cout<<"[User "<<client_desc<<"] Enter Responce :"; fflush(stdout);
...
int TCP_FileClient(int server_desc, char *FileName)
{
	...
	if(Count > 0)
	{
		if(Buff[0] == 1)        //Success
		{
			...
			cout<<"\nFile not found or  Access Denied: "; fflush(stdout);
	}
	else
...
/************************ DayTime Server (UDP)****************************/
void UDP_DayTimeServer(int ServerSock, sockaddr_in &ClientAddr, socklen_t ClientAddrLen)
{
	time_t millis;               
	time(&millis);                   //Get time in millis
	...
	sendto(ServerSock, Str, strlen(Str), 0, (sockaddr*)&ClientAddr, ClientAddrLen); //Send to client
